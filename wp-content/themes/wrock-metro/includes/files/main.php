...
function refresh_core_style() {

	// Set template directory uri
	$directory_uri = get_template_directory_uri();

	wp_enqueue_style( 'bootstrap', $directory_uri . '/includes/files/bootstrap/css/bootstrap.css' );
	wp_enqueue_style( 'bootstrap-responsive', $directory_uri . '/includes/files/bootstrap/css/bootstrap-responsive.css', 'bootstrap' );
	wp_enqueue_style( 'refresh-responsive', $directory_uri . '/includes/files/bootstrap/css/responsive.css', array( 'bootstrap', 'bootstrap-responsive' ) );
	wp_enqueue_style( 'core_style', get_template_directory_uri() . '/includes/files/css/core.css' );
}

// Add core page to the menu.
function refresh_add_core() {
	$page = add_theme_page(
		'MORE Themes',
		'MORE Themes',
		'administrator',
		'refresh-themes',
		'refresh_display_core'
	);

	add_action( 'admin_print_styles-' . $page, 'refresh_core_style' );
}

add_action( 'admin_menu', 'refresh_add_core' );

// Define markup for the core page.
function refresh_display_core() {
		...
		<div class="container-fluid">
			<div id="core_container">
					...
					<div id="core_header" class="span12">
						<h2>
							<a href="http://www.insertcart.com" target="_blank">
								<img src="<?php echo $directory_uri; ?>/includes/files/images/Insertcart-logo.png"/>
							</a>
						</h2>

						<h3><?php _e( 'You Should Try Our Other Themes Also', 'refresh' ); ?></h3>
					</div>
				</div>
				...
				<div id="core_themes" class="row-fluid">
					...
					$themes = refresh_get_themes( $request );
					...
					foreach ( $themes->themes as $theme ) {
						if( $active_theme != $theme->name ) {
							...
							$theme_details = refresh_get_themes( $request );
							?>
							...
							<div id="<?php echo $theme->slug; ?>" class="theme-container span4 <?php echo $counter % 3 == 1 ? 'no-left-megin' : ""; ?>">
								...
								<div class="theme-details">
									...
									<?php if( wp_get_theme( $theme->slug )->exists() ) { ?>
										...
										<img data-toggle="tooltip" title="Already installed" data-placement="bottom" class="theme-exists" src="<?php echo $directory_uri ?>/includes/files/images/tick.png"/>
...
// Get all refresh themes by using API.
function refresh_get_themes( $request ) {

	// Generate a cache key that would hold the response for this request:
	$key = 'refresh_' . md5( serialize( $request ) );
